...
def convert_volume():
    ...
    for coin in volatile_coins:
        ...
        free = float(free_balance['free']) * 1
...
def current():
    ...
    LastPricea = client.get_symbol_ticker(symbol='AAVEUSDT') 
...
def maxprice():
    ...
    LastPriceb = client.get_symbol_ticker(symbol='AAVEUSDT')
        ...
        if (coin not in coins_bought and float(lastpriceb) > last_sell_static * 1.001 and float(lastpriceb) <= maxpricea * 0.9999) or (coin not in coins_bought and last_sell_static > currentprice and currentprice > last_sell * 1.015) :
            ...
            # run the else block if the position has been placed and return order info
                ...
                while orders[coin] == []:
                    ...
                    boughtat_a = client.get_symbol_ticker(symbol='AAVEUSDT')
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if (LastPrice <= (maxpricea * 0.9992) and LastPrice >= (BuyPrice * 1.0018)) or (LastPrice <= BuyPrice * 0.99 ):
            ...
            # run the else block if coin has been sold and create a dict for each coin sold
                ...
                if LOG_TRADES:
                    ...
                    with open('lastsellstatic.txt', 'w') as filehandle:
                        for listitem in sell:
                            filehandle.write('%s' % listitem)
                    profits_file = str(f"\n{datetime.now().replace(microsecond=0).isoformat(' ')}, {coin} , {coins_sold[coin]['volume']}, {BuyPrice}, {LastPrice}, {profit:.2f}, {PriceChange-(TRADING_FEE*2):.2f} \n")
