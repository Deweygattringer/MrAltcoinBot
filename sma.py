        ...
        if threshold_check < CHANGE_IN_PRICE:
            ...
            if datetime.now() >= volatility_cooloff[coin] + timedelta(minutes=TIME_DIFFERENCE):
                ...
                if len(coins_bought) + len(volatile_coins) < MAX_COINS or MAX_COINS == 0:
                    ...
                    print(f'{txcolors.WARNING}{coin} has gained {round(threshold_check, 3)}% within the last {TIME_DIFFERENCE} minutes, but already bought in{txcolors.DEFAULT}')
...
#    volume_bnb = float(0.03)
 ...
 #       buy_limit = client.create_order(
   ...
   #     print('bnb successfully added')
    ...
    symbol = 'TRXBUSD'
    starttime = '8 hours ago UTC'  # to start for 1 week ago
    interval = '5m'
    ...
    symbol_df['20sma'] = symbol_df['close'].rolling(20).mean()
    ...
    symbol_df['Signal'] = np.where(symbol_df['5sma'] > symbol_df['20sma'], 1, 0) # ggf. hier buylogik adaptieren u.a
    ...
    signalpos = (outputline[-4])
    print(f'{txcolors.WARNING}SMA SIGNAL {signalvalue}  Position: {signalpos}  \n')
    print(f'{txcolors.BLUE}5-SMA: {threesma} / 20-SMA: {fifteensma} / current price: {Price}')
    #PLOT GRAPHS IF WANTED:
    
    # df=df.astype(float)
    # df[['close', '5sma','20sma']].plot()
      ...
      #          if LOG_TRADES:
       #             write_log(f"Buy : {volume[coin]} {coin} - {last_price[coin]['price']}")
        ...
        #        continue
            ...
            print(f'There is already an active trade on {coin}, or buy parameters are not met')
