...
def convert_volume():
    '''Converts the volume in free BUSD to the coin's volume'''
    ...
    for coin in volatile_coins:
        ...
        # calculate the volume in coin from QUANTITY in BUSD (default)
...
def buy():
    ...
    #avarage prices
    historical_trx = client.get_symbol_ticker(symbol='TRXBUSD')
    pricetrx = historical_trx['price']
    histrxprice = float(historical_trx['price'])
    hstprice = str(histrxprice)
    price_avg = client.get_avg_price(symbol='TRXBUSD')
    ...
    orders = {}
    currenttrx = client.get_symbol_ticker(symbol='TRXBUSD') 
    current = currenttrx['price']
    currentprice = float(current)
    LastPriceb = client.get_symbol_ticker(symbol='TRXBUSD')
...
# Hier neuer bear code
    ...
    for coin in volume:
        ...
        buyneutral = (coin not in coins_bought and price_change >= (-0.9) and last_sell_static >= currentprice and currentprice >= last_sell_static * 0.99 and currentprice >= last_sell * 1.0012 and currentprice > avg)
        ...
        buybull2 = (coin not in coins_bought and price_change >= (-0.9) and currentprice < maxpricea and currentprice >= last_sell * 1.001 and currentprice >= maxpricea * 0.994 and currentprice >= avg * 1.0012)
        ...
        if buyafterstoploss:
            ...
            # run the else block if the position has been placed and return order info
                ...
                while orders[coin] == []:
                    ...
                    boughtat_a = client.get_symbol_ticker(symbol='TRXBUSD')
                    ...
                    #    Log trade
                    if LOG_TRADES:
                        write_log(f"I busd just bought: {volume[coin]} {coin} @ {last_price[coin]['price']}")
