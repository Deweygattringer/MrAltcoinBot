...
def wait_for_price():
    ...
    if historical_prices[hsp_head]['IOTA' + PAIR_WITH]['time'] > datetime.now() - timedelta(minutes=float(TIME_DIFFERENCE / RECHECK_INTERVAL)):

        # sleep for exactly the amount of time required
        time.sleep((timedelta(minutes=float(TIME_DIFFERENCE / RECHECK_INTERVAL)) - (datetime.now() - historical_prices[hsp_head]['IOTA' + PAIR_WITH]['time'])).total_seconds())
...
def convert_volume():
    ...
    for coin in volatile_coins:
        ...
        free = float(free_balance['free']) * 0.5
...
def current():
    ...
    LastPricea = client.get_symbol_ticker(symbol='IOTAUSDT') 
...
def maxprice():
    ...
    LastPriceb = client.get_symbol_ticker(symbol='IOTAUSDT')
...
# Hier neuer bear code
    ...
    for coin in volume:
        ...
        if (coin not in coins_bought and float(lastpriceb) > last_sell_static and float(lastpriceb) <= maxpricea * 0.9995) or (coin not in coins_bought and last_sell_static > currentprice and currentprice > last_sell * 1.015) :
            ...
            # run the else block if the position has been placed and return order info
                ...
                while orders[coin] == []:
                    ...
                    boughtat_a = client.get_symbol_ticker(symbol='IOTAUSDT')
